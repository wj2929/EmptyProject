namespace Microsoft.VisualStudio.TextTemplating.CodeDom
{
    ...
    internal static class CodeDomHelpers
    {
        ...
        {
            member.Comments.Add(new CodeCommentStatement(new CodeComment("<summary>", true)));
            member.Comments.Add(new CodeCommentStatement(new CodeComment(comment, true)));
            member.Comments.Add(new CodeCommentStatement(new CodeComment("</summary>", true)));
        }
        ...
        {
            return new CodeConditionStatement(new CodeBinaryOperatorExpression(new CodeVariableReferenceExpression(name), CodeBinaryOperatorType.IdentityEquality, nullEx), new CodeStatement[] { new CodeThrowExceptionStatement(typeof(ArgumentNullException).New(new CodeExpression[] { name.Prim() })) });
        }
        ...
        internal static CodeMemberMethod CreateMethod(Type returnType, string name, string summaryComment, MemberAttributes attributes, params CodeObject[] statements)
        {
            CodeMemberMethod member = new CodeMemberMethod {
                ReturnType = (returnType == null) ? null : returnType.Ref(),
                Name = name
            };
            member.AddSummaryComment(summaryComment);
            member.Attributes = attributes;
            foreach (CodeObject obj2 in statements)
            {
        ...
        {
            return new CodeTypeReferenceExpression(type) { Type = { Options = CodeTypeReferenceOptions.GlobalReference } };
        }
        ...
        {
            return new CodeConditionStatement(new CodeBinaryOperatorExpression(variableRef, CodeBinaryOperatorType.IdentityEquality, nullEx), new CodeStatement[] { new CodeAssignStatement(variableRef, type.New()) });
        }
        ...
        {
            return new CodeObjectCreateExpression(type.Ref(), new CodeExpression[0]);
        }
